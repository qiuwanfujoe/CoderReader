...
- (BOOL)expectsRequestBodyFromMethod:(NSString *)method atPath:(NSString *)path
{
	...
	if([method isEqualToString:@"POST"] && [path isEqualToString:@"/upload.html"]) {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"BeginUploadFiles" object:nil userInfo:nil];
...
- (NSObject<HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
	...
	if ([method isEqualToString:@"POST"] && [path isEqualToString:@"/upload.html"])
	{
        ...
        NSDictionary *userInfo = [NSDictionary dictionaryWithObject:uploadedFiles forKey:@"uploadedFiles"];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"DidUploadFiles" object:nil userInfo:userInfo];
		return [[HTTPDynamicFileResponse alloc] initWithFilePath:templatePath forConnection:self separator:@"%" replacementDictionary:replacementDict];
	}
	if( [method isEqualToString:@"GET"] && [path hasPrefix:@"/upload/"] ) {
		// let download the uploaded files
		return [[HTTPFileResponse alloc] initWithFilePath: [kDocumentDir stringByAppendingString:path] forConnection:self];
	}
...
- (void) processStartOfPartWithHeader:(MultipartMessageHeader*) header {
	...
	NSString* uploadDirPath = [kDocumentDir stringByAppendingPathComponent:@"upload"];
